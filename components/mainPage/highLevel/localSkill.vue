	...
	<div>
		...
		<a-card :bordered="false">
			<a-form class="ant-advanced-search-form" id="searchForm"  @submit="submitForm">
				<a-row :gutter="24" :class="cardClass" class="search-box" >
					<a-col :span="6" >
						<a-form-item label="姓名" :label-col="{ span: 9 }" :wrapper-col="{ span: 15 }">
						    <a-input  placeholder="请输入姓名" v-model="formModel.name"/>
					    </a-form-item>					
					</a-col>
					<a-col :span="6">
						<a-form-item label="工作单位所在地" :label-col="{ span: 9 }">
						    <a-col :span="7">
						    	<a-select 
									:loading="cityLoad" 
									v-model="formModel.nowUnitProvince"
									@change="nowCity"
								>
									<a-select-option 
										:value="cityItem.addr" 
						    ...
						    <a-col :span="7">
						    	<a-select 
									:loading="nowLoad"
									v-model="formModel.nowUnitCity"
								>
									<a-select-option 
										v-for="(item,key) in nowList"
										:value="item.addr" 
									>
							          	{{item.addr}}
							        </a-select-option>
					...
					<a-col :span="6">
						<a-form-item label="毕业院校" :label-col="{ span: 9 }" :wrapper-col="{ span: 15 }">
					...
					<a-col :span="6">
						<a-form-item label="毕业省份" :label-col="{ span: 9 }" :wrapper-col="{ span: 15 }">
						    <a-select 
								:loading="cityLoad" 
					...
					<a-col :span="6" >
						<a-form-item label="研究领域" :label-col="{ span: 9 }" :wrapper-col="{ span: 15 }">
						    <a-input  placeholder="请输入研究领域" v-model="formModel.field"/>
					    </a-form-item>					
					</a-col>
					
					<a-col :span="6">
						<a-form-item label="籍贯" :label-col="{ span: 9 }">
						    <a-col :span="7">
						    	<a-select 
									:loading="cityLoad" 
					...
					<a-col :span="6">
						<a-form-item label="年龄范围" :label-col="{ span: 9 }">
						    <a-col :span="7"><a-input v-model="formModel.startAge"/></a-col>
						    <a-col :span="1" align="center">-</a-col>
						    <a-col :span="7"><a-input v-model="formModel.endAge"/></a-col>
					    </a-form-item>					
					</a-col>
				</a-row>
		...
		>
			<router-link 
				...
				slot="name" 
				slot-scope="text,val" 
				:to="{name:'highTalentsView',query:{'id':val.id,'goBack':'本地人才','url':url}}"
			>
          		{{text}}
          	</router-link>
		</a-table>-->
		<wTable
			:columns="columns"
			tableUrl="getLocalTalentByConditionAndPage"
			:model="formModel"
			title="本地人才"
			ref="mychild"
			:toHref="toHref"
...
const columns = [
	{
	    ...
	    title: '工作单位省份',
	    dataIndex: 'nowUnitProvince',
	    key: 'nowUnitProvince',
	    align:'center'
  	},{
	    title: '工作单位地市',
	    dataIndex: 'nowUnitCity',
	    key: 'nowUnitCity',
	    align:'center'
  	}
];
export default{
	data(){
		return{
			url:"",
      		columns,
      		toHref:{name:'highTalentsView',query:{'goBack':'本地人才','url':this.url}},
	...
	methods:{
		...
		area(name,opt){
			this.cityChange(opt,"area","birthCity");
		},
		nowCity(name,opt){
			this.cityChange(opt,"now","nowUnitCity");
		},
		cityChange(opt,list,model){//name必须有
			if(opt.key){//非不限选项
				var load = list+"Load",
					list = list+"List";
				...
				.then(response => {
					...
					if(response.length > 1){//非直辖市时，全省搜索
						this[list].unshift({"addr":"不限"});
					}
					this.formModel[model] =this[list][0].addr;
				},err => {
					err = this.common.errorAjax(err);
					if(err){
				this.$message.error(err);
