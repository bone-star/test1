	...
	<div>
					...
					<a-col :span="6">
						...
						>
						    <a-select  v-model="formModel.talentLevel">
						        <a-select-option v-for="item in formList.talentLevelList" :value="item" >
						          	{{item}}
						        </a-select-option>
						    </a-select>
					    </a-form-item>					
		...
		>
			<router-link 
				...
				slot="name" 
				slot-scope="text,val" 
				:to="{name:'highTalentsView',query:{'id':val.id,'goBack':'高层次人才','url':url}}"
			>
          		{{text}}
          	</router-link>
		</a-table>-->
		<wTable
			:columns="columns"
			tableUrl="getHighTalentByConditionAndPage"
			:model="formModel"
			title="高层次人才"
			ref="mychild"
			:toHref="toHref"
...
export default{
	data(){
		return{
			url:"",
			toHref:{name:'highTalentsView',query:{'goBack':'高层次人才','url':this.url}},
