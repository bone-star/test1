<template>
	<div>
		<!--<a-breadcrumb>
		    ...
		    <a-breadcrumb-item>院士工作站</a-breadcrumb-item>
		</a-breadcrumb>-->
				...
				<div :class="cardClass" class="search-box" >
					<a-row :gutter="24" >
						<a-col :md="{span:8}" :lg="{span:6}" >
							<a-form-item label="进驻院士姓名" :label-col="{ span: 9 }" :wrapper-col="{ span: 15}">
						...
						<a-col :md="{span:8}" :lg="{span:6}">
							<a-form-item label="技术领域" :label-col="{ span: 9 }"  :wrapper-col="{ span: 15}">
								<a-input  placeholder="请输入技术领域" v-model="formModel.field"/>
							</a-form-item>
						</a-col>
						<a-col :md="{span:8}" :lg="{span:6}">
...
export default{
	...
	methods:{
		viewUrl(name){
			//console.log(name)
			var obj = {"name":name};
			this.backstage.viewUrl(this,obj,"院士工作站","academician")
		},
		searchReset(){
			this.formModel = {
				"name":"",
		...
		tableList(){
			...
			let url = "findAcadeWorkStationByConditionAndPage";
			let _this = this;
