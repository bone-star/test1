<template>
	<div>
		<!--<a-breadcrumb>
		    ...
		    <a-breadcrumb-item>广东自然基金项目</a-breadcrumb-item>
		</a-breadcrumb>-->
					...
					<a-row :gutter="24" >
						...
						<a-col :md="{span:8}" :lg="{span:6}">
							<a-form-item label="年份" :label-col="{ span: 9 }"  :wrapper-col="{ span: 15}">
								<a-select defaultValue="年份" v-model="formModel.proDate">
...
export default{
	...
	methods:{
		viewUrl(name){
			//console.log(name)
			var obj = {"name":name};
			this.backstage.viewUrl(this,obj,"广东自然基金","NFProject");
		},
		searchReset(){
			this.formModel = {
				"itemName":"",
		...
		tableList(){
			...
			let url = "findNsfcItemByConditionAndPage";
			let _this = this;
