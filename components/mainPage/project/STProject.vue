<template>
	<div>
		<!--<a-breadcrumb>
		    ...
		    <a-breadcrumb-item>广东科技项目</a-breadcrumb-item>
		</a-breadcrumb>-->
					...
					<a-row :gutter="24" >
						...
						<a-col :md="{span:8}" :lg="{span:6}" >
							<a-form-item label="承担单位名称" :label-col="{ span: 9 }" :wrapper-col="{ span: 15}">
							    <a-input  placeholder="请输入承担单位名称" v-model="formModel.bearUnit"/>
						    </a-form-item>					
						</a-col>
						<a-col :md="{span:8}" :lg="{span:6}">
							<a-form-item label="下达时间" :label-col="{ span: 9 }">
								<a-col :span="7">
									<a-input placeholder="0000" suffix="年" v-model="formModel.startDate"/>
								</a-col>
						...
						<a-col :md="{span:8}" :lg="{span:6}">
							<a-form-item label="立项金额" :label-col="{ span: 9 }">
...
const columns = [
	{
	    ...
	    title: '立项金额(万元)',
	    dataIndex: 'proApprovalMoney',
	    key: 'proApprovalMoney',
	    align:'center' 
  	},{
	    title: '下达时间',
...
export default{
	...
	methods:{
		viewUrl(name){
			//console.log(name)
			var obj = {"name":name};
			this.backstage.viewUrl(this,obj,"广东科技","STProject")
		},
		searchReset(){
			this.formModel = {
				"name":"",
		...
		tableList(){
			...
			let url = "findItemInfoByConditionAndPage";
			let _this = this;
